...
function [fbmov_int, fbmov_int_c, fbmov_int_down_st, fbmov_int_up_st] = feedbackmov_int(plotBool, cursorPosX, cursorPosY, indPeak, offset, numTrials, wrong_trial, upTrials, downTrials, exTrials, peTrials, subID)
%%%%%%%%%%%%%%%%%%%%%%% Movement Length %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
mov_int = zeros(numTrials,1);
for i = 1:numTrials
    if wrong_trial(i) == 0
        mov_int(i) = sum(sqrt(diff(cursorPosX{i,1}(indPeak(i):offset(i))).^2 + diff(cursorPosY{i,1}(indPeak(i):offset(i))).^2)) * 100; %movement length in cm
    else
        mov_int(i) = NaN;
    end
end
