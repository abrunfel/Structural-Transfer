...
% Set wrong_trial = 1 if onset > offset
wrong_trial(onset > offset) = 1;

% 3/4/22 UPDATE: Now that we are analyzing training phase data for the
% random rotations, we need "theta" to reflect the actual rotation amounts.
% Make sure to check that IDE values are appropriate!
...
for i = 1:numTrials
    rot(i) = sortData(i).rotation_amount(1); % Change to theta after this loop runs
end
theta = rot;
...
save([subID '_train_postStep2' '.mat'],'sortData','downTrials','upTrials','subID','onset','offset','wrong_trial',...
