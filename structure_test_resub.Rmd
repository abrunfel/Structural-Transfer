...
#setwd("") # Mac - exp1
#setwd("") # Mac - exp2
#setwd("Z:\\Data\\UURAF Projects\\UURAF 2019 Structural Learning and Transfer\\struct_learn_paper\\Post_Step_3_resub") # PC
setwd("//Volumes//mnl//Data//UURAF Projects//UURAF 2019 Structural Learning and Transfer//struct_learn_paper//Post_Step_3_resub") # Mac
...
outliers = c(18) # Subject 18 is a lefty!
...
testData_mbb = testData %>% group_by(subjectID, group, block) %>%
  ...
  #select(-c(trial, target_theta, wrong_trial)) # for whatever reason, this breaks the code (likely something with an update to R or dyplyr or both?)
...
#keyboard # break code before running stats; you can unncomment to continue
aov.ide = aov_ez(subset(testData_mbb, testData_mbb$block %in% c(5,14)), id = 'subjectID', dv = 'ide.bc', between = 'group', within = 'block')
aov.rmse = aov_ez(subset(testData_mbb, testData_mbb$block %in% c(5,14)), id = 'subjectID', dv = 'fbrmse.bc', between = 'group', within = 'block')
aov.mov_int = aov_ez(subset(testData_mbb, testData_mbb$block %in% c(5,14)), id = 'subjectID', dv = 'fbmov_int.bc', between = 'group', within = "block")
aov.MT = aov_ez(subset(testData_mbb, testData_mbb$block %in% c(5,14)), id = 'subjectID', dv = 'fbMT_c', between = 'group', within = "block")
aov.norm_jerk = aov_ez(subset(testData_mbb, testData_mbb$block %in% c(5,14)), id = 'subjectID', dv = 'fbnorm_jerk.bc', between = 'group', within = "block")

aov.ide$anova_table
aov.rmse$anova_table
aov.mov_int$anova_table
aov.MT$anova_table
aov.norm_jerk$anova_table
# If you want partial eta squared, use this
#aov_ez(subset(testData_mbb, testData_mbb$block %in% c(5,14)), id = 'subjectID', dv = 'fbmov_int.bc', between = 'group', within = "block", anova_table = list(es = 'pes'))
```

```{r t-test at first block exposure}
t.test(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$ide.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$ide.bc, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbrmse.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbrmse.bc, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbmov_int.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbmov_int.bc, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbMT_c,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbMT_c, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbnorm_jerk.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbnorm_jerk.bc, paired = FALSE)
```
```{r Cohen's d at first block exposure}
cohen.d(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$ide.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$ide.bc, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbrmse.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbrmse.bc, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbmov_int.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbmov_int.bc, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbMT_c,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbMT_c, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(1))$fbnorm_jerk.bc,
       subset(testData_mbb, testData_mbb$block %in% c(5) & testData_mbb$group %in% c(2))$fbnorm_jerk.bc, paired = FALSE)
```

```{r t-test at last block exposure}
t.test(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$ide.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$ide.bc, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbrmse.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbrmse.bc, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbmov_int.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbmov_int.bc, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbMT_c,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbMT_c, paired = FALSE)

t.test(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbnorm_jerk.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbnorm_jerk.bc, paired = FALSE)
```

```{r Cohen's d at last block exposure}
cohen.d(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$ide.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$ide.bc, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbrmse.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbrmse.bc, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbmov_int.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbmov_int.bc, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbMT_c,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbMT_c, paired = FALSE)

cohen.d(subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(1))$fbnorm_jerk.bc,
       subset(testData_mbb, testData_mbb$block %in% c(14) & testData_mbb$group %in% c(2))$fbnorm_jerk.bc, paired = FALSE)
```

```{r t-test at first block post-exposure}
t.test(subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(1))$ide.bc,
       subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(2))$ide.bc, paired = FALSE)
t.test(subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(1))$fbrmse.bc,
       subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(2))$fbrmse.bc, paired = FALSE)
t.test(subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(1))$fbmov_int.bc,
       subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(2))$fbmov_int.bc, paired = FALSE)
t.test(subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(1))$fbMT_c,
       subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(2))$fbMT_c, paired = FALSE)
t.test(subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(1))$fbnorm_jerk.bc,
       subset(testData_mbb, testData_mbb$block %in% c(15) & testData_mbb$group %in% c(2))$fbnorm_jerk.bc, paired = FALSE)
```