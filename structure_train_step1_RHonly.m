...
if strcmp(str,'MACI64') == 1
    cd('/Volumes/mnl/Data/UURAF Projects/UURAF 2019 Structural Learning and Transfer/Raw_Data_RH');
    fname = uigetfile('*train.zip');
    fname = fname(1:end);
else
    cd('Z:\Data\UURAF Projects\UURAF 2019 Structural Learning and Transfer\Raw_Data_RH');
    fname = uigetfile('*train.zip');
    fname = fname(1:end);
end
...
% Fix for erroneous trial 17 generated in simulink/stateflow.
theta(vbTrials) = 0; % rotation in degrees during exposure phase
for i = 17:256
theta(exTrials(i-16)) = sortData(i).rotation_amount(1); % NOTE: uses the specific rotation amount for particular trial
end
theta(peTrials) = 0;
...
%wrong_trial(exTrials) = 0; % Set wrong_trial during exposure trials to 1
%so no data are processed during this phase. NO: we are analyzing the
%training phase now; need to comment this out!
...
% figure(1);
ang = 0:0.1:2.01*pi;
r = sortData(i).TARGET_TABLE.Visual_radius(2); % Target target size
r_home = sortData(1).TARGET_TABLE.Visual_radius(1); % Home target size
for i = 1:numTrials
    if wrong_trial(i) == 0 % Since wrong_trial = 1 for exposure trials, the code will not plot these and force onset(i) and offset(i) = NaN
        flag = 2;
        while flag>1
            figure('Position', [0 0 1920/2 1080/2]); %[bottom left corner coords X and Y, W, H]
            subplot(1,2,1)
            plot(sortData(i).TARGET_TABLE.X(1)+r_home*cos(ang),sortData(i).TARGET_TABLE.Y(1)+r_home*sin(ang),'Color',[255/255 117/255 56/255])
            hold on
            plot(sortData(i).TARGET_TABLE.X(2)+r*cos(ang),sortData(i).TARGET_TABLE.Y(2)+r*sin(ang),'k')
            hold on
            plot(sortData(i).TARGET_TABLE.X(3)+r*cos(ang),sortData(i).TARGET_TABLE.Y(3)+r*sin(ang),'k')
            axis([sortData(i).TARGET_TABLE.X(2)-20 sortData(i).TARGET_TABLE.X(2)+20 sortData(i).TARGET_TABLE.Y(3)-10 sortData(i).TARGET_TABLE.Y(2)+10]);
            axis square;
            ...
            if sortData(i).TRIAL.TP == 1 || sortData(i).TRIAL.TP == 2 % Non-rotated Trials
            ...
            elseif sortData(i).TRIAL.TP == 3 || sortData(i).TRIAL.TP == 4 % Rotated Trials need to show cursor position, not hand position
                ...
                hold on
                plot(cursorPosX{i,1}(offset(i,1)-1)*100-Tx,cursorPosY{i,1}(offset(i,1)-1)*100-Ty,'mo'); % had to put in "-1" to fix index overflow
            end
            subplot(1,2,2)
            plot(velR{i,1});
            hold on
            ...
            hold on
            plot(offset(i,1)-2,velR{i,1}(offset(i,1)-2), 'mo'); % had to put in "-2" to fix index overflow
...
%switch Directory
if strcmp(str,'MACI64') == 1
    cd('/Volumes/mnl/Data/UURAF Projects/UURAF 2019 Structural Learning and Transfer/Post_Step_1_RH');
else
    cd('Z:\Data\UURAF Projects\UURAF 2019 Structural Learning and Transfer\Post_Step_1_RH');
end
...
save([filename '_postStep1' '.mat'],'sortData');
save([filename '_postStep1' '.mat'],'onset', 'offset', 'wrong_trial', '-append');
if strcmp(str,'MACI64') == 1
    cd('/Volumes/mnl/Data/UURAF Projects/UURAF 2019 Structural Learning and Transfer/matlab');
else
    cd('Z:\Data\UURAF Projects\UURAF 2019 Structural Learning and Transfer\matlab');
end