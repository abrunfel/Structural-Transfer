...
dir_list = dir('*train*.mat');    %Store subject *mat data file names in variable (struct array).
...
for B = 1:1:A2
   ...
   if strcmp(subID(8:10),'CON') == 1 %if subject file name includes 'CON' in the 8th-10th letters, then it belongs to the control group
       group1 = 1;
   elseif strcmp(subID(8:10),'STR') == 1 %if subject file name includes 'CON' in the 8th-10th letters, then it belongs to the STR group
       group1 = 2;
   else
...
%With this code, a file is created where the above 33 variables are stored
%for each subject for the pre-exposure baseline and the post-exposure
%baseline

% if strcmp(str,'MACI64') == 1
%     cd('/Volumes/mnl/Data/UURAF Projects/UURAF 2019 Structural Learning and Transfer/Post_Step_3_RH');
%     dlmwrite('train_raw', ALL_subjects, '-append', 'delimiter', ',', 'precision','%.6f');
%     cd('/Volumes/mnl/Data/UURAF Projects/UURAF 2019 Structural Learning and Transfer/Post_Step_2_RH');
% else
