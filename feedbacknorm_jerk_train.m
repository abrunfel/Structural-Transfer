...
function [fbnorm_jerk, fbnorm_jerk_c, fbnorm_jerk_down_st, fbnorm_jerk_up_st] = feedbacknorm_jerk_train(plotBool, sortData, indPeak, offset, numTrials, wrong_trial, upTrials, downTrials, exTrials, peTrials, subID, fs, fbMT, fbmov_int)
%%%%%%%%%%%%%%%%%%%%%%% Normalized Jerk %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:numTrials
    if wrong_trial(i) == 0
        acc_tan{i,1} = 100*sqrt((sortData(i).Right_HandXAcc(indPeak(i):offset(i)-1)).^2 + (sortData(i).Right_HandYAcc(indPeak(i):offset(i)-1)).^2); % in cm/s/s % NOTE: the original code did not segment to onset/offset. I've added it here; "-1" to fix index overflow
