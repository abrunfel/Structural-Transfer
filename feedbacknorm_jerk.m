...
function [fbnorm_jerk, fbnorm_jerk_c, fbnorm_jerk_down_st, fbnorm_jerk_up_st] = feedbacknorm_jerk(plotBool, sortData, indPeak, offset, numTrials, wrong_trial, upTrials, downTrials, exTrials, peTrials, subID, fs, fbMT, fbmov_int)
%%%%%%%%%%%%%%%%%%%%%%% Normalized Jerk %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:numTrials
    if wrong_trial(i) == 0
        acc_tan{i,1} = 100*sqrt((sortData(i).Left_HandXAcc(indPeak(i):offset(i))).^2 + (sortData(i).Left_HandYAcc(indPeak(i):offset(i))).^2); % in cm/s/s % NOTE: the original code did not segment to onset/offset. I've added it here
